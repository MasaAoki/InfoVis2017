        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    s0 = volume.values[x + vid0[0] + (y + vid0[1])*lines +  (z + vid0[2])*slices][0];
                    s1 = volume.values[x + vid1[0] + (y + vid1[1])*lines +  (z + vid1[2])*slices][0];
                    s2 = volume.values[x + vid2[0] + (y + vid2[1])*lines +  (z + vid2[2])*slices][0];
                    s3 = volume.values[x + vid3[0] + (y + vid3[1])*lines +  (z + vid3[2])*slices][0];
                    s4 = volume.values[x + vid4[0] + (y + vid4[1])*lines +  (z + vid4[2])*slices][0];
                    s5 = volume.values[x + vid5[0] + (y + vid5[1])*lines +  (z + vid5[2])*slices][0];
